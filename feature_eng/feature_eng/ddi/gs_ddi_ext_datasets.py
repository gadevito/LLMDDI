...
def main(input_pickle, drugbank_pickle, output_pickle, known_ddi):
    ...
    possible_pairs = [
        (drug1, drug2) for drug1, drug2 in itertools.combinations(all_drug_ids, 2)
        if (drug1, drug2) not in all_known_interactions and (drug2, drug1) not in all_known_interactions
    ]
    #random.shuffle(possible_pairs)

    with tqdm(total= num_positives, desc="Generating true negatives") as pbar:
        for drug1_id, drug2_id in possible_pairs:
            if len(dataset) >= 2 * num_positives:
                break
            drug1 = drugbank_drug_dict[drug1_id]
            drug2 = drugbank_drug_dict[drug2_id]
            # Avoinding overlap with the all_know_interactions dataset, but it should not happen
            ...
            if not sanity_check(target_vector1) or not sanity_check(target_vector2):
                print("Found an interaction without targeted genes in negatives")
...
if __name__ == "__main__":
    ...
    for d in datasets:
        ...
        main(file_name, args.drugbank_pickle, output_pickle, args.known_ddi)
