...
def classify_interaction(interaction):
    prompt = """Interaction between drugs:  
{interaction}  

Main classification: [Pharmacokinetic / Pharmacodynamic]  
...
Timing:"""
    ...
    res = {"main":None, "Pharmacokinetic":None, "Pharmacodynamic": None, "Severity": None, "Timing":None}
    for l in lines:
        if (i := l.find("Main classification:")) !=-1:
            j = len("Main classification:")
            res["main"] = l[i+j:].strip().lower()
    ...
    #logger.debug(res)
    if res["main"] is None or (res["main"] and res["main"] not in ('pharmacokinetic','pharmacodynamic')):
        raise ValueError("Error in classification")
...
def update_interaction_classification(csv_file, batch_size, wait_time):
    try:
        ...
        required_columns = ["interaction_classification", "pharmacokinetic_effect",
        ...
        def process_row(index, row):
            if pd.isna(row["interaction_classification"]):
                description = row.get("Interaction_Description", "")
                classification = classify_interaction(description)
                return index, classification
            #logger.debug(f"process_row {index}")
            return index, {"main": row["interaction_classification"], "Pharmacokinetic":row["pharmacokinetic_effect"],
            ...
            for i in range(num_batches):
                ...
                with ThreadPoolExecutor(max_workers=10) as executor:
                    ...
                    for future in as_completed(futures):
                        ...
                        if classification:
                            df.at[index, "interaction_classification"] = classification["main"]
