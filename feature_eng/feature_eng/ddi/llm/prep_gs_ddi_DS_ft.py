#
# The following script opens the the dataset, and creates the jsonl files to fine-tune deepseek r1 distilled models.
# 
...
def jsonl_batch(drug):
    ...
    json_data = {
                    "messages": [
                        {"role": "system", "content": SYSTEM_DDI_CLASSIFICATION_DS},
...
def classify(drug, model="gpt-4o",backoff_factor=1.0):
    for attempt in range(5):
        try:
            ...
            genes1 =", ".join(drug['genes1'])
            genes2 =", ".join(drug['genes2'])
            msg = [{"role": "system", "content": SYSTEM_DDI_CLASSIFICATION_DS}]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create the dataset for fine-tuning Deepseek r1 distilled models.")
