#
# The following script opens a dataset and uses Gemini model to classify the ddi
...
def classify(sel, drug, model="gemini-1.5-pro",backoff_factor=1.0, random_examples=False, num_examples=10):
    for attempt in range(5):
        try:
            ...
            response = client.models.generate_content(
                model=model,
                contents=types.Part.from_text(USER_DDI_CLASSIFICATION_RC.format(examples=examples,drug1=drug1, smiles1=smiles1, org1=org1, genes1=genes1, drug2=drug2, smiles2=smiles2, org2=org2, genes2=genes2)),
                config=types.GenerateContentConfig(
                    system_instruction=SYSTEM_DDI_CLASSIFICATION_RC,
                    temperature= 0,
                ),
            )
            ...
            if cleaned_text.startswith("no interaction"):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='gemini-1.5-pro', help='Model to use for the classification.')
